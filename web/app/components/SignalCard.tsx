...
export default function SignalCard({ signal, isLoading }: SignalCardProps) {
  ...
  useEffect(() => {
    ...
    const updateTimer = () => {
      try {
        ...
        if (isNaN(expireAt.getTime()) || isNaN(enterAt.getTime())) {
          setIsExpired(true)
    ...
    return (
      <div className="bg-gray-900 rounded-xl shadow-2xl border border-gray-700 p-8 text-center">
        <div className="text-gray-300 mb-4">
          ...
          <p className="text-lg font-medium text-white">{t('getting_signal')}</p>
          <p className="text-sm text-gray-400">{t('loading_data')}</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className={`bg-[#0f1419] rounded-xl shadow-2xl border-2 p-6 ${
      isUp ? 'border-green-500/30' : 'border-red-500/30'
    }`}>
      {/* Signal Direction Badge - Психологические цвета */}
      <div className="flex justify-center mb-6">
        <div className={`px-8 py-4 rounded-lg font-medium text-white flex items-center gap-3 shadow-lg ${
          isUp ? 'bg-green-600/90 hover:bg-green-600' : 'bg-red-600/90 hover:bg-red-600'
        } transition-all duration-200`}>
          <Icon className="h-7 w-7" />
          <span className="text-xl font-light tracking-wide">{signal.direction}</span>
        </div>
      </div>

      {/* Symbol and Timeframe */}
      <div className="text-center mb-8">
        <h2 className="text-3xl font-light text-white mb-3 tracking-tight">{signal.symbol}</h2>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="bg-[#1a1f2e] rounded-lg p-4 border border-gray-800">
          <div className="text-xs text-gray-500 mb-2 uppercase tracking-wider font-light">{t('enter_time')}</div>
        ...
        <div className="bg-[#1a1f2e] rounded-lg p-4 border border-gray-800">
          <div className="text-xs text-gray-500 mb-2 uppercase tracking-wider font-light">{t('expire_time')}</div>
          <div className="font-light text-white text-base">
            {(() => {
              try {
      ...
      <div className={`flex items-center justify-between p-4 rounded-lg text-white border-2 ${
        isExpired 
        ...
        <div className="flex items-center gap-3">
          {isExpired ? (
            <>
              ...
              <span className="text-gray-400 font-light">{t('signal_expired')}</span>
            </>
          ) : (
            <>
              <ClockIcon className={`h-5 w-5 ${isUp ? 'text-green-400' : 'text-red-400'}`} />
              <span className="text-white font-light text-base">{timeLeft}</span>
            </>
          )}
        </div>
        ...
        {!isExpired && (
          <div className={`text-xs font-light uppercase tracking-wider ${
            ...
            {new Date(signal.enter_at) <= new Date() ? t('signal_active') : 'Pending'}
          </div>
        )}
      </div>
      ...
      <div className="mt-4 flex items-center justify-center gap-2">
        <div className={`w-2 h-2 rounded-full animate-pulse ${
          isExpired ? 'bg-gray-600' : (isUp ? 'bg-green-500' : 'bg-red-500')
        }`}></div>
        <span className={`text-xs font-light uppercase tracking-wider ${
          isExpired ? 'text-gray-500' : (isUp ? 'text-green-400' : 'text-red-400')
        }`}>
          {isExpired ? t('signal_completed') : t('signal_active')}
        </span>
      </div>
    </div>
  )

...
function SafeTradingViewWidget({ 
  ...
  const loadWidget = useCallback(async () => {
    ...
    try {
      ...
      let script = document.querySelector('script[src*="embed-widget-advanced-chart.js"]') as HTMLScriptElement
            ...
            try {
              ...
              if (container.current && container.current.parentNode && widgetContainer.parentNode) {
                ...
                if (typeof (window as any).TradingView !== 'undefined') {
                  widgetInstanceRef.current = new (window as any).TradingView.widget(config)
                } else {
                  // Fallback - создаем скрипт с конфигурацией
                  const configScript = document.createElement('script')
                  configScript.type = 'text/javascript'
                  configScript.innerHTML = `new TradingView.widget(${JSON.stringify(config)});`

  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const accessResponse = await fetch(`${apiUrl}/api/auth/can-access-signals`, {
        ...
        const data = await response.json()
        setError(data.detail || (language === 'ru' ? 'Неверный email или пароль' : 'Invalid email or password'))
      }
    } catch (err) {

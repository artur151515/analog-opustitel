...
import IframeTradingViewWidget from "../components/IframeTradingViewWidget";
...
function SignalsIframePageContent() {
	const router = useRouter();
	const searchParams = useSearchParams();
	const [selectedSymbol, setSelectedSymbol] =
		useState("EURUSD");
	const [selectedTimeframe, setSelectedTimeframe] =
		useState(searchParams?.get("tf") || "5m");
	...
	useEffect(() => {
		const checkAuth = () => {
			try {
				...
				const pocketId = localStorage.getItem(
					"pocket_option_id",
				);
				const balance = localStorage.getItem(
					"user_balance",
				);
				const isVerified = localStorage.getItem(
					"is_verified",
				);
				const userTier =
	...
	const { data: stats, error: statsError } =
		useSWR(
			isAuthorized
				? `/api/stats?symbol=${selectedSymbol}&tf=${selectedTimeframe}`
				: null,
			fetcher,
			{
	...
	return (
		<div className="min-h-screen bg-[#0a0e1a] text-white">
					...
					<div className="flex items-center justify-between h-16">
						<div className="flex items-center space-x-4">
							...
							<h1 className="text-xl font-light tracking-tight">
								{translate({
									ru: "Торговые сигналы (iframe)",
									en: "Trading signals (iframe)",
								})}
							</h1>
						</div>
			...
			<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
				...
				<div className="mb-8">
					<div
						className="bg-gray-800 rounded-xl shadow-lg border border-gray-700 p-0 overflow-hidden"
						style={{ height: "500px" }}
					>
						<IframeTradingViewWidget
							symbol={selectedSymbol}
							timeframe={selectedTimeframe.replace(
								"m",
								"",
							)}
						/>
					...
					{signalError && (
						<div className="bg-red-900/20 border border-red-800 rounded-xl p-8 text-center">
							<p className="text-red-400">
								{translate({
									ru: "Ошибка загрузки сигнала",
									en: "Failed to load signal",
								})}
							</p>
						</div>
					)}
							...
							<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
								<div className="text-center">
									<p className="text-2xl font-bold text-green-400">
										{stats.winrate_last_n}%
									</p>
									<p className="text-gray-400">
										{translate({
											ru: `Win-rate (последние ${stats.n})`,
											en: `Win rate (last ${stats.n})`,
										})}
									</p>
								</div>
								<div className="text-center">
...
export default function SignalsIframePage() {
	return (
		<LanguageProvider>
			<Suspense
				fallback={
